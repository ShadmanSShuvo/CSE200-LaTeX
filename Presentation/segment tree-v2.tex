...
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}]
  ...
  % Top-down construction: start from the whole range
  \onslide<1->{
  ...
  % Finally, reach individual elements
  \onslide<4->{
  ...
  % Root divides into two ranges
  \onslide<2->{
  \draw[segedge] (n1)--(n2);
  \draw[segedge] (n1)--(n3);}

  % Each range divides again
  \onslide<3->{
  \draw[segedge] (n2)--(n4);
  \draw[segedge] (n2)--(n5);
  \draw[segedge] (n3)--(n6);
  ...
  % Finally divide into individual elements
  \onslide<4->{
  \draw[segedge] (n4)--(l1);
  \draw[segedge] (n4)--(l2);
  \draw[segedge] (n5)--(l3);
...
\onslide<4->{\centering\small Built top-down: each range is recursively \bignav{divided} into two smaller halves.}
...
\begin{tikzpicture}[scale=0.5, every node/.style={transform shape}]
  \foreach \v/\i in {2/1,1/2,5/3,3/4,7/5,4/6,6/7,8/8}{
    ...
    \fi
    \node[font=\tiny] at (\x+0.375,0.375) {\v};
    \node[font=\tiny, text=navy!60] at (\x+0.375,-0.22) {\i};
  }
\end{tikzpicture}%
}%
\only<4->{%
