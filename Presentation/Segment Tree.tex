...
%  PART I -- INTRODUCTION & MOTIVATION (Wasif)
% ============================================================
...
% PART I -- INTRODUCTION & MOTIVATION (Wasif)
% ============================================================
\setpresenter{Wasif --- Introduction \& Motivation}
...
\begin{tikzpicture}[scale=0.75, every node/.style={transform shape, font=\scriptsize}]
  ...
  \onslide<2->{\draw[thick, warningred] plot[domain=0.2:4.5, samples=30] (\x, {\x});}
  \onslide<2->{\node[text=warningred, font=\tiny] at (4.3,3.6) {touch everything};}
  \onslide<3->{\draw[thick, accentgreen] plot[domain=0.2:4.5, samples=30] (\x, {ln(\x+1)});}
  \onslide<3->{\node[text=accentgreen, font=\tiny\bfseries] at (4.3,1.7) {split \& combine};}
\end{tikzpicture}
\end{column}
\end{columns}
\vspace{0.3cm}
\onslide<3->{\centering\small That gap between the two curves --- \textbf{that's} the whole idea we're chasing.}
\end{frame}
...
% ---------------- SLIDE: ANATOMY ----------------
\begin{frame}{Anatomy of a Segment Tree}
\vspace{-0.15cm}
\begin{center}
\begin{tikzpicture}[scale=0.92, every node/.style={transform shape}]
  ...
  \onslide<2->{\draw[segedge] (n4)--(l1); \draw[segedge] (n4)--(l2);
  \draw[segedge] (n5)--(l3); \draw[segedge] (n5)--(l4);
  \draw[segedge] (n6)--(l5); \draw[segedge] (n6)--(l6);
  \draw[segedge] (n7)--(l7); \draw[segedge] (n7)--(l8);}
  \onslide<3->{\draw[segedge] (n2)--(n4); \draw[segedge] (n2)--(n5);
  \draw[segedge] (n3)--(n6); \draw[segedge] (n3)--(n7);}
  \onslide<4->{\draw[segedge] (n1)--(n2); \draw[segedge] (n1)--(n3);}
  \foreach \x/\i in {0.8/1, 2.6/2, 4.4/3, 6.2/4, 7.8/5, 9.6/6, 11.4/7, 13.2/8}
    \node[font=\tiny, text=navy!70] at (\x, 0.75) {\i};
\end{tikzpicture}
\end{center}
\onslide<4->{\centering\small Built bottom-up: each parent is just the \bignav{merge} of its two children.}
% ADDED: pins the O(log n) height claim directly on Shayan's own anatomy slide
\onslide<4->{
\vspace{2pt}
\begin{center}
{\scriptsize\textcolor{navy!70}{\textbf{Height $= \lceil \log_2 n \rceil$} $\;\Rightarrow\;$ every root-to-leaf path is just $O(\log n)$ long.}}
\end{center}
}
\end{frame}
...
% ADDED: explicit boxed complexity statement, owned entirely by this slide
\onslide<8->{
\vspace{3pt}
\begin{center}
\begin{tikzpicture}
...
% ---------------- SLIDE: THREE CASES ----------------
\begin{frame}{Three Ways to Answer a Query}
\centering
\vspace{0.2cm}
\begin{tikzpicture}[every node/.style={font=\small}, node distance=1.2cm]
  \onslide<1->{
  \node[draw=warningred, thick, fill=warningred!10, rounded corners=4pt,
        minimum width=4.2cm, minimum height=1.4cm, align=center] (c1) at (-4.8,0)
    {\textbf{No overlap}\\ \scriptsize query and node don't touch\\ \textbf{return identity (0)}};}
  \onslide<2->{
  \node[draw=accentgreen, thick, fill=softgreen, rounded corners=4pt,
        minimum width=4.2cm, minimum height=1.4cm, align=center] (c2) at (0,0)
    {\textbf{Full overlap}\\ \scriptsize node lies entirely inside query\\ \textbf{return stored value}};}
  \onslide<3->{
  \node[draw=gold, thick, fill=gold!15, rounded corners=4pt,
        minimum width=4.2cm, minimum height=1.4cm, align=center] (c3) at (4.8,0)
    {\textbf{Partial overlap}\\ \scriptsize node straddles the boundary\\ \textbf{recurse into both children}};}
\end{tikzpicture}
\vspace{0.4cm}

\onslide<3->{\small Only \textbf{partial-overlap} nodes ever recurse further --- and there are just $O(\log n)$ of them per query.}
% ADDED: one-line "why" so the claim isn't just asserted but justified
\onslide<3->{
\vspace{4pt}
\centering
{\scriptsize\textcolor{navy!70}{Why: partial-overlap nodes only ever occur along the two boundary paths of the tree --- at most $2\log n$ of them.}}
}
\end{frame}

% ---------------- SLIDE: QUERY WALKTHROUGH (SIGNATURE) ----------------
\begin{frame}{Watching sum(3,7) Unfold}
\vspace{-0.2cm}
\begin{center}
\begin{tikzpicture}[scale=0.78, every node/.style={transform shape}]
  ...
  \onslide<5->{
    \node[segnode, segpath] (n2r) at (3.5,4.5) {[1,4]\\ \textbf{8}};
  ...
  \onslide<4->{
    \node[segnode, segfull] (n5) at (5.3,3) {[3,4]\\ \textbf{8}};
    \node[segleaf] (l3) at (4.4,1.3) {5};
    \node[segleaf] (l4) at (6.2,1.3) {3};
    ...
    \draw[-{Latex[length=2mm]}, thick, accentgreen, bend left=35, shorten >=1pt, shorten <=1pt]
      (n5.north) to node[midway, sloped, above, font=\scriptsize\bfseries, text=accentgreen]{$+8$} (n2.south);
  }

  % ---- STEP 5: [1,4] resolves, gold return to root ----
  \onslide<5->{
    \draw[-{Latex[length=2mm]}, thick, gold, bend right=30, shorten >=1pt, shorten <=1pt]
      (n2r.north) to node[midway, sloped, above, font=\scriptsize\bfseries, text=darkgold]{$+8$} (n1.south);
  }
  ...
  \onslide<4-6>{\node[font=\Large, text=navy] at (7,-1) {$0 + 8$};}
  \onslide<7-8>{\node[font=\Large, text=navy] at (7,-1) {$0 + 8 + 11$};}
  \onslide<9-11>{\node[font=\Large, text=navy] at (7,-1) {$0 + 8 + 11 + 6 + 0$};}
  \onslide<12->{
...
\end{tikzpicture}
\end{center}
% ADDED: closing complexity punchline tying the whole trace together
\onslide<12->{
\vspace{-2pt}
\begin{center}
{\scriptsize\textcolor{navy!70}{Only \textbf{7} of the tree's 15 nodes were ever visited --- for any $n$, this scales as $O(\log n)$.}}
\end{center}
}
\end{frame}
...
% ============================================================
% PART III -- TIME COMPLEXITY, LIMITS & REAL WORLD (Shuvo)
% ============================================================
\setpresenter{Shuvo --- Complexity, Extensions \& Applications}
