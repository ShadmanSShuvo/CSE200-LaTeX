...
\author{Shayan Un Noor (2305024) \\ Md.\ Shadman Shahriyar Shuvo (2305025) \\ Md.\ Wasif Haque (2305029)}
\date{CSE200 --- Data Structures \& Algorithms}
...
The tree is built bottom-up: leaves are assigned the array values directly, and every internal node is computed as the merge of its two children.
...
Only $7$ of the tree's $15$ nodes were ever visited to answer this query; the remaining $8$ nodes (e.g.\ the entire leaf $8$ and the subtree beneath $[1,2]$ once it is classified as disjoint) are pruned without further recursion.
...
Addition satisfies this since $(a+b)+c = a+(b+c)$, and so do $\min$ and $\max$, since $\min(\min(a,b),c) = \min(a,\min(b,c))$ and likewise for $\max$. Table~\ref{tab:generalization} lists several associative operations that can be substituted directly into the same tree structure.
...
For a live leaderboard of player scores, a Segment Tree over sorted score buckets supports rank queries and $k$-th-largest lookups in $O(\log n)$, with score updates handled just as fast.

\subsection{Scheduling and Booking}
For booking systems, a Segment Tree over seat or time-slot availability can locate or reserve a free contiguous range in $O(\log n)$, rather than scanning every slot.
...
A Fenwick Tree (Binary Indexed Tree) supports both point update and range-sum query in $O(\log n)$, with a smaller memory footprint ($\sim n$) and a simpler implementation than a Segment Tree. However, it is naturally restricted to invertible, sum-like operations, and range updates (via lazy propagation) are considerably more awkward to implement than in a Segment Tree.
...
Build                    & $O(n)$      & $O(n)$ (with the linear-time build) \\
...
As a concrete illustration of why this matters at scale: for a system with $n = 1024$ instruments processing $q = 36 \times 10^9$ combined queries and updates at roughly one nanosecond per elementary step, a na\"ive $O(qn)$ approach requires approximately $10.24$ hours of computation, whereas a Segment Tree's $O(q \log n)$ approach completes the same workload in roughly $6$ minutes --- a difference of more than two orders of magnitude driven entirely by the gap between $n$ and $\log_2 n$.
